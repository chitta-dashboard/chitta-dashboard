...
const RESOLUTIONS_DATA = "RESOLUTIONS_DATA";
...
const ResolutionsProvider: React.FC<PropType> = ({ children }) => {
  ...
  const { data: ResolutionsData } = useQuery(["resolutions"], () => axios.get("http://localhost:5001/resolutions"));

  useEffect(() => {
    ResolutionsData && fetchResolutions(ResolutionsData?.data);
  }, [ResolutionsData]);

  const fetchResolutions = (data: IResolution) => {
    dispatch({ type: RESOLUTIONS_DATA, payload: data });
  };
